...
/*   Updated: 2026/08/14 23:42:53 by zheng            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line_bonus.h"

static void	add_buffer_to_line(t_line *buffer, t_line *line)
{
	size_t	i;
	char	*new_content;
	size_t	chunk_len;
	...
	i = -1;
	while (++i < chunk_len)
		new_content[line->len + i] = buffer->content[buffer->start + i];
...
char	*get_next_line(int fd)
{
		...
		|| (line.len > 0 && line.content[line.len - 1] != '\n'))
	{
		...
		if (status == 0)
			break ;
		buffer[fd].end = get_len(&buffer[fd]);
		add_buffer_to_line(&buffer[fd], &line);
		if (!line.content)
			return (NULL);
